...
\begin{itemize}
	\item Symmetric and separable in $X$ and $Y$ (traded) and numeraire
		\begin{itemize}
...
\begin{itemize}[<+->]
	...
	\item Dept. of Commerce / Int'l Trade Commission ($G$)
		\begin{itemize}[<+->]
			...
			\item Susceptible to influence of lobbying, perhaps both directly and indirectly
...
\pause
\vskip.2in
\begin{beamerboxesrounded}[upper=palette tertiary, shadow=true]{Assumptions}
\begin{enumerate}
  \pause
	\item $\ga(e,\ta)$ is increasing and concave in $e$ for all $\ta \in \Theta$.
	\item $\ga(e,\ta)$ is increasing in $\ta$.
\end{enumerate}
...
\begin{enumerate}
  \pause
	\item   The pdf of the induced distribution on $\ga(e,\ta)$ is weakly increasing in $e$.
\end{enumerate}
\end{beamerboxesrounded}
...
\pause
\vskip.25in
\begin{beamerboxesrounded}[upper=palette tertiary, shadow=true]{Lemma}
...
\vskip.2in
\pause
Indirect effect is ambiguous: higher $\tau^{\textit{ad}}$ can increase or decrease incentive to exert lobbying effort

\vskip.2in
\pause
	The total probability that $G$ renews AD duties only increases if indirect effect is positive / large enough to outweigh direct effect.
\begin{itemize}
	\item Work in progress: endogenize $\tau^{\textit{ad}}$
\end{itemize}
...
\begin{itemize}[<+->]
	...
	\item Lobbying incentives are muted
\end{itemize}

\vskip.25in
\pause
\begin{beamerboxesrounded}[upper=palette tertiary, shadow=true]{Result 4}
  The total probability that $G$ renews AD duties increases when the weighting function shifts up exogenously if the direct effect dominates.
\end{beamerboxesrounded}
\end{frame}
