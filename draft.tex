...
\begin{lemma}
  The direct effect of exogenous positive shifts in the political weighting function $\ga(\cdot,\ta)$ on the probability the government renews the antidumping duty is positive. \hyperlink{Pr_eg}{Proof}
	  \label{res:eg}
\end{lemma}
...
\vskip.4in	  	
\noindent \textbf{\hypertarget{Pr_eg}{Proof of Result~\ref{res:eg}}}: \\
...
\begin{equation}
  \frac{\left(\pi_X(\tau^a) - \pi_X(\tad)\right)\left(\frac{\partial }{\partial \pi(\dot)} \left[\pi_X(\tad) - \pi_X(\tau^a) \right]\right) + Z(\bta,\tad) \left(\frac{\partial }{\partial \pi(\dot)} \left[\pi_X(\tad) - \pi_X(\tau^a) \right]\right)}{\left(\pi_X(\tau^a) - \pi_X(\tad)\right)^2}.
  \label{appex:pi}
\end{equation}
