...
\begin{lemma}
  The direct effect of the home trade agreement tariff on the probability the government renews the antidumping duty is negative.
  \label{res:leg_a}
\end{lemma}
...
\begin{lemma}
  Lobbying effort is weakly decreasing in trade agreement tariffs.
  \label{res:lobby}
\end{lemma}
...
\begin{lemma}
  The probability that the government agency renews the antidumping duties is increasing and concave in lobbying effort $\left(\text{i.e. } \frac{\partial r}{\partial e} \geq 0, \ \frac{\partial^2 r}{\partial e^2} \leq 0  \right)$.
  \label{res:bincC}
\end{lemma}
...
\begin{result}
  The total probability that the antidumping duties will be renewed is constant in the foreign trade agreement tariff.
  \label{res:leg_astar}
\end{result}
...
\begin{lemma}
  The direct effect of exogenous positive shifts in the political weighting function $\ga(\cdot,\ta)$ on the probability the government renews the antidumping duty is positive.
	  \label{res:eg}
\end{lemma}
...
The effect on the renewal probability is determined by the sign of the derivative of the left hand side of the inequality in Expression~\ref{eq:b_ex} with respect to $\tau^a$; to show that the break probability is decreasing in $\tau^a$, I must demonstrate that this derivative is positive. Labeling the numerator of that expression $Z(\tad,\bta)$, the derivative of this quantity with respect to $\tau^{a}$ is 
\begin{equation}
  \frac{\left(\pi_X(\tad) - \pi_X(\tau^a)\right)\left(\frac{\partial \mathit{CS}_X(\tau^{a})}{\partial \tau^{a}} + \frac{\partial \pi_X(\tau^a)}{\partial \tau^{a}} + \frac{\partial \mathit{TR}(\tau^{a})}{\partial \tau^{a}}\right) - Z(\tad,\bta) \frac{\partial \pi_X(\tau^a)}{\partial \tau^a}}{\left(\pi_X(\tau^a) - \pi_X(\tad)\right)^2}.
  \label{appex:a}
\end{equation}
...
Because total social welfare is maximized at $\tau^a = \tau^{*a} = 0$, $Z(\tad,\bta)$ is always negative, whereas profits are increasing in $\tau^a$, so the second summand is positive everywhere (inclusive of the leading negative sign). With a positive denominator, we thus have that the derivative is positive on $[0,\tau^O]$.
...
Since the denominator is a squared term of a strictly positive quantity, the expression in Equation~\ref{appex:a} is positive for all $\tau^{a}$.
$\hfill\blacksquare$
...
Proof is via the Implicit Function Theorem using the lobby's first order condition, Equation~\ref{eq:lobbyfoc}, referred to here as $FOC_L$.
  \[
		\frac{\partial e}{\partial \tau^a} = - \frac{\frac{\partial FOC_L}{\partial \tau^a}}{\frac{\partial FOC_L}{\partial e}} = \frac{ \frac{\partial r}{\partial e} \frac{\partial \pi(\bta)}{\partial \tau^a} - \frac{\partial^2 r}{\partial e \partial \tau^a} \left[ \pi(\tad) - \pi(\tau^a) \right]}{\frac{\partial^2 r}{\partial e^2} \left[ \pi(\tad) - \pi(\tau^a) \right]}
	\]
...
Turning to the numerator: $\frac{\partial r}{\partial e}$ is positive and $\frac{\partial \pi(\bta)}{\partial \tau^a}$ is positive by construction so the first term is positive. 

We can rewrite $\frac{\partial r(e,\bta)}{\partial  \tau^a} = -\frac{\partial F_\ga (Z(\tad,\bta))}{\partial \tau^a} = -\frac{\partial F_\ga (Z(\tad,\bta))}{\partial Z(\tad,\bta)} \frac{\partial Z(\tad,\bta)}{\partial \tau^a} = - f_\ga \frac{\partial Z(\tad,\bta)}{\partial \tau^a}$, where $F_\ga (Z(\tad,\bta))$ is the CDF of $\ga$ and $f_\ga (Z(\tad,\bta))$ is the pdf of $\ga$ and $Z(\tad,\bta)$ again represents the left hand side of the inequality in Expression~\ref{eq:b_ex}.
	
Then $\frac{\partial^2 r}{\partial e \partial \tau^a} = 	- \frac{\partial}{\partial e} \left( \frac{\partial F_\ga (Z(\tad,\bta))}{\partial \tau^a} \right) = 
	   - f_\ga \frac{\partial^2 Z(\tad,\bta)}{\partial \tau^a \partial e} - [\frac{\partial Z(\tad,\bta)}{\partial \tau^a} \frac{\partial f_\ga}{\partial e} = - \frac{\partial Z(\tad,\bta)}{\partial \tau^a} \frac{\partial f_\ga}{\partial e} $
where the above equality holds because $Z(\tad,\bta)$ does not depend on $e$. The proof of Lemma~\ref{res:leg_a} shows that $\frac{\partial Z(\tad,\bta)}{\partial \tau^a} \geq 0$, so $\frac{\partial f_\ga}{\partial e} \geq 0$ ensures that $\frac{\partial^2 r}{\partial e \partial \tau^a} \leq 0$. Thus $\frac{\partial e}{\partial \tau^a} \leq 0$.  $\hfill\blacksquare$


\vskip.4in
\noindent \textbf{\hypertarget{Pr_bincC}{Proof of Lemma~\ref{res:bincC}}}:\\
\noindent The left side of the inequality in Expression~\ref{eq:b_ex}, $Z(\tad,\bta)$, does not depend on $e$. Thus we have $r(e,\bta,\tad) = \Pr [Z < \ga(e,\ve) ] = 1 - F_{\ga}(\ga=Z)$ where $F_{\ga}(\ga=Z) = F_{\ta}(\ta=h^{-1}(\ga,e))$ by the Change of Variables Theorem and Assumption~\ref{as:ga_ta} with $\ga = h(e,\ta)$ giving the change of variable.
