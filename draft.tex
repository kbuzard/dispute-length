...
where the above equality holds because the pdf of $\ta$ does not depend on $e$. When $\ga(\cdot)$ shifts up, $\frac{\partial h^{-1}(\ga,e)}{\partial \ga(\cdot)}$ becomes less negative, so $\frac{\partial^2 r}{\partial e \partial \ga(\cdot)} \leq 0$.
