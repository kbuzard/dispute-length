...
In the first phase, when no temporary trade barrier is in place, an industry exerts effort to convince the ITC to enact a temporary trade barrier for its product. The ITC then must decide whether to maintain the MFN tariff or impose the temporary trade barrier. Importantly, we assume that the decisions of the ITC are not deterministic, with the amount of uncertainty varying by industry.

Given that the industry receives temporary protection in the first phase of interaction, there will be another phase of interaction when the trade barrier expires, here modeled as five years later to match the typical five years sunset provision under the WTO. At this point, the interaction is similar to that during the first phase, with the industry first exerting effort and then the ITC deciding whether to extend the temporary trade barrier or to revert to the MFN tariff. The essential strategic difference is that at this point the level of the temporary trade barrier is taken as exogenously set at the level determined in the first round of interaction.
