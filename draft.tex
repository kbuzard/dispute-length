...
The effect on the renewal probability is determined by the sign of the derivative of the left hand side of the inequality in Expression~\ref{eq:b_ex} with respect to $\tad$; to show that the renewal probability is decreasing in $\tad$, I must demonstrate that this derivative is positive. Again labeling the numerator of that expression $Z(\bta,\tad)$, the derivative of this quantity with respect to $\tad$ is 

\begin{equation}
  \frac{\left(\pi_X(\tau^a) - \pi_X(\tad)\right)\left(\frac{\partial \mathit{CS}_X(\tad)}{\partial \tad} + \frac{\partial \pi_X(\tad)}{\partial \tad} + \frac{\partial \mathit{TR}(\tad)}{\partial \tad}\right) + Z(\bta,\tad) \frac{\partial \pi_X(\tad)}{\partial \tad}}{\left(\pi_X(\tau^a) - \pi_X(\tad)\right)^2}.
  \label{appex:tad}
\end{equation}
...
$Z(\bta,\tad)$ is also always positive on $[0,\tau^O]$ since $\tad > \tau^a$ and on this range $\tad$ is closer to the optimum. Profits are increasing in $\tad$, so the second summand is positive on $[0,\tau^O]$. With a positive denominator, we thus have that the entire express is positive on $[0,\tau^O]$.
...
where the above equality holds because $Z(\bta,\tad)$ does not depend on $e$. As long as $\tad$ is greater than the optimal unilateral tariff, $\frac{\partial Z(\bta,\tad)}{\partial \tad} \leq 0$, so $\frac{\partial f_\ga}{\partial e} \geq 0$ ensures that $\frac{\partial^2 r}{\partial e \partial \tad} \geq 0$. Thus $\frac{\partial e}{\partial \tad} \leq 0$.  $\hfill\blacksquare$
...
The denominator is negative as show in the Proof of Lemma~\ref{res:lobby}. The first term in the numerator is zero since the profit function does not vary in $\ga(\cdot,\cdot)$.
...
\begin{equation}
  \frac{\left(\pi_X(\tau^a) - \pi_X(\tad)\right)\left(\frac{\partial }{\partial \pi(\cdot)} \left[\pi_X(\tad) - \pi_X(\tau^a) \right]\right) + Z(\bta,\tad) \left(\frac{\partial }{\partial \pi(\cdot)} \left[\pi_X(\tad) - \pi_X(\tau^a) \right]\right)}{\left(\pi_X(\tau^a) - \pi_X(\tad)\right)^2}.
  \label{appex:pi}
\end{equation}
...
%\noindent $\left(\pi_X(\tad) - \pi_X(\tau^a)\right)$ is always positive. 
...
Turning to the numerator: $\frac{\partial r}{\partial e}$ is positive and $\frac{\partial }{\partial \pi(\cdot)} \left[\pi_X(\tad) - \pi_X(\tau^a) \right]$ is positive by assumption so the first term is positive. 
...
where the above equality holds because $Z(\bta,\tad)$ does not depend on $e$. $-\frac{\partial Z(\bta,\tad)}{\partial \pi(
cdot)} = - \left(\frac{\partial }{\partial \pi(\cdot)} \left[\pi_X(\tad) - \pi_X(\tau^a) \right]\right) \leq 0$ by assumption, so $\frac{\partial f_\ga}{\partial e} \geq 0$ ensures that $\frac{\partial^2 r}{\partial e \partial \pi(\cdot)} \leq 0$. Thus the sign of $\frac{\partial e}{\partial \pi(\cdot)}$ is ambiguous.  $\hfill\blacksquare$


\vskip.4in	  	
\noindent \textbf{\hypertarget{Pr_pi_total}{Proof of Result~\ref{res:pi_total}}}: \\
$\frac{\partial R(\bta,\tad)}{\partial \pi(\cdot)} = \frac{\partial r}{\partial  e}\frac{\partial e}{\partial \pi(\cdot)} + \frac{\partial r}{\partial \pi(\cdot)}$. $\frac{\partial r}{\partial  e}$ is non-negative by Lemma~\ref{res:bincC}. $\frac{\partial e}{\partial \pi(\cdot)}$ is non-negative by Lemma~\ref{res:pi_indir}. $\frac{\partial r}{\partial \pi(\cdot)}$ is non-negative by Lemma~\ref{res:pi_dir}. Thus the entire expression is non-negative. $\hfill\blacksquare$
