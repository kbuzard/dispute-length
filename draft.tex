...
\begin{multline}
  r(e,\bta,\btad) = \expect_{\ga|e} \bm{1} [ W_G(\btad,\ga(e,\ve)) > W_G\left(\bta,\ga(e,\ve)\right) ] \\ = \Pr [ W_G(\btad,\ga(e,\ve)) > W_G\left(\bta,\ga(e,\ve)\right) | e]
  \label{eq:b}
\end{multline}

The lobby chooses its level of effort as a function of the trade agreement tariffs and the antidumping duties, given the implications of that choice on the government agency's probability of breaking the agreement. The lobby maximizes probability-weighted profits net of effort:
\[
  \max_{e} r(e,\bta,\btad) \pi(\tau^{ad}) + [1 - r(e,\bta,\btad)] \pi(\tau^a) - e
\]
...
\begin{equation}
	\frac{\partial r(e,\bta,\btad)}{\partial e} \left[ \pi(\tau^{ad})  - \pi(\tau^a) \right] = 1 
	\label{eq:lobbyfoc}
\end{equation}
...
I represent the probability that the trade agreement will be broken as $R(\bta)=r(e(\bta),\bta)$ where $e(\bta)$ is the best response function implicit in Equation \ref{eq:lobbyfoc}, the lobby's first order condition.
