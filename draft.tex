...
\begin{lemma}
  The direct effect of the home trade agreement tariff on the probability the legislature renews the antidumping duty is negative.
  \label{res:leg_a}
\end{lemma}
...
\begin{result}
	The total probability that the trade agreement will be broken is decreasing in $\bta$ (i.e. $\frac{\partial R(\bta)}{\partial  \tau^a} \leq 0$).
	\label{res:bcomB}
\end{result}
...
Proof is via the Implicit Function Theorem using the lobby's first order condition, Equation~\ref{eq:lobbyfoc}, referred to here as $FOC_L$.
  \[
		\frac{\partial e}{\partial \tau^a} = - \frac{\frac{\partial FOC_L}{\partial \tau^a}}{\frac{\partial FOC_L}{\partial e}} = \frac{ \frac{\partial b}{\partial e} \frac{\partial \pi(\bta)}{\partial \tau^a} - \frac{\partial^2 b}{\partial e \partial \tau^a} \left[ \pi(\tad) - \pi(\tau^a) \right]}{\frac{\partial^2 b}{\partial e^2} \left[ \pi(\tad) - \pi(\tau^a) \right]}
	\]
Beginning with the denominator: $\tad > \tau^a$ and $\pi(\cdot)$ is increasing in $\tau$ so the expression is brackets is positive. $\frac{\partial^2 b}{\partial e^2}$ is negative by Result~\ref{res:bincC}, so the denominator is negative.

Turning to the numerator: $\frac{\partial b}{\partial e_b}$ is positive and $\frac{\partial \pi(\bta)}{\partial \tau^a}$ is positive by construction so the first term is positive. 
...
Then $\frac{\partial^2 b}{\partial e_b \partial \tau^a} = 	- \frac{\partial}{\partial e} \left( \frac{\partial F_\ga (Z(\tad,\bta))}{\partial \tau^a} \right) = 
