...
where the above equality holds because $Z(\bta,\tad)$ does not depend on $e$. $-\frac{\partial Z(\bta,\tad)}{\partial \pi(\cdot)} = - \left(\frac{\partial }{\partial \pi(\cdot)} \left[\pi_X(\tad) - \pi_X(\tau^a) \right]\right) \leq 0$ by assumption, so $\frac{\partial f_\ga}{\partial e} \geq 0$ ensures that $\frac{\partial^2 r}{\partial e \partial \pi(\cdot)} \leq 0$. Thus the sign of $\frac{\partial e}{\partial \pi(\cdot)}$ is ambiguous.  $\hfill\blacksquare$
