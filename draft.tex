...
where the above equality holds because $Z(\bta,\tad)$ does not depend on $e$. Refer to result in proof of Lemma~\ref{res:pi_dir}; then $\frac{\partial f_\ga}{\partial e} \geq 0$ implies that $\frac{\partial^2 r}{\partial e \partial \pi(\cdot)} \ldots$. Thus the sign of $\frac{\partial e}{\partial \pi(\cdot)}$ is ambiguous.  $\hfill\blacksquare$
