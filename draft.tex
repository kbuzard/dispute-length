...
The effect on the renewal probability is determined by the sign of the derivative of the left hand side of the inequality in Expression~\ref{eq:b_ex} with respect to $\tau^a$; to show that the break probability is decreasing in $\tau^a$, I must demonstrate that this derivative is positive. Labeling the numerator of that expression $\left[ W(\tad,\tau^{*a})-W(\bta)\right]$ (for the change in social welfare), derivative of this quantity with respect to $\tau^{a}$ is 
\begin{equation}
  \frac{\left(\pi_X(\tad) - \pi_X(\tau^a)\right)\left(\frac{\partial \mathit{CS}_X(\tau^{a})}{\partial \tau^{a}} + \frac{\partial \pi_X(\tau^a)}{\partial \tau^{a}} + \frac{\partial \mathit{TR}(\tau^{a})}{\partial \tau^{a}}\right) - \left[ W(\tad,\tau^{*a})-W(\bta)\right] \frac{\partial \pi_X(\tau^a)}{\partial \tau^a}}{\left(\pi_X(\tau^a) - \pi_X(\tad)\right)^2}.
  \label{appex:a}
\end{equation}
...
Because total social welfare is maximized at $\tau^a = \tau^{*a} = 0$, $W(\tad,\tau^{*a})-W(\bta)$ is always negative, whereas profits are increasing in $\tau^a$, so the second summand is positive everywhere (inclusive of the leading negative sign). With a positive denominator, we thus have that the derivative is positive on $[0,\tau^O]$.
