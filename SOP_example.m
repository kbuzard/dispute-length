%Going to adapt SOP-RIE example for no retaliation
...
lhs = (8*(1.25 + (c).^a)-5).*(tn-ta) + (4*(1.25 + (c).^a)-34).*(tn^2-ta.^2);

%add e to gamma
e1=.25; e2=.25;
up = (8*(1.25 + (c).^a +e1)-5).*(tn-ta) + (4*(1.25 + (c).^a +e1)-34).*(tn^2-ta.^2);
low = (8*(1.25 + (c).^a -e2)-5).*(tn-ta) + (4*(1.25 + (c).^a -e2)-34).*(tn^2-ta.^2);
spread = up - low;
b = up./spread;
for j=1:tad
    if lhs(1,j)< 0
